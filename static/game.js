...
socket.on('ended', () => { location.reload(); });

function start() {
    socket.emit('start', {guesser: getGuesser(), game_name: game_name});
}

function displaySecretWord(secret_word, guesser) {
    var disp = `${secret_word}\n(${guesser} is the guesser)`;
    if (getName() === guesser) {
        is_guesser = true;
        disp = 'You are the guesser!';
    }
    document.getElementById('secretWord').innerHTML = disp;
...
socket.on('started', (data) => {
    ...
    displaySecretWord(data.secret_word, data.guesser);
});
...
socket.on('unveil clues', (data) => {
    ...
    if (!document.getElementById('approvingStatus').innerHTML) {    
        document.getElementById('approvingStatus').innerHTML =
...
socket.on('guessing', (data) => {
    ...
    if (is_guesser) {
        playNotification();
        document.getElementById('secretWord').innerHTML = 'You are the guesser!';
